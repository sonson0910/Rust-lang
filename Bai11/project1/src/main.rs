...
impl Students{
    ...
    fn remove(&mut self, name: &str) -> bool{
        self.class.remove(name).is_some() // is_some: co gia tri tra ve true, nguoc lai
    }
...
let mut students = Students::new();
    loop{
        Manager::show();
        let input = get_input().expect("Please enter your data"); // expect: ko co gia tri se print mot gia tri nao do
        match Manager::choice(&input.as_str()){
            ...
            Some(Manager::DetaleStudent) => manager::remove_student(&mut students),
            None => break,
        }
    }
}
